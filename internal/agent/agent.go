...
func (a Agent) Evaluate(task domain.Task, input string) domain.AnswerResult {
	...
	case "single_command", "fill_blank":
		if norm == exp || strings.HasPrefix(norm, exp+" ") { r.ScoreDelta = r.Exact; r.Notes = "správně" } else { r.ScoreDelta = r.Wrong; r.Notes = "špatně" }
	case "multi_command":
		if (strings.Contains(norm, "ip a") && strings.Contains(norm, "ping")) || strings.Contains(norm, "2>&1") { r.ScoreDelta = r.Exact; r.Notes = "správně" } else if strings.Contains(norm, "ip") || strings.Contains(norm, "ping") { r.ScoreDelta = r.Partial; r.Notes = "částečně správně" } else { r.ScoreDelta = r.Wrong; r.Notes = "špatně" }
	case "multiple_choice":
		if norm == exp { r.ScoreDelta = r.Exact; r.Notes = "správně" } else { r.ScoreDelta = r.Wrong; r.Notes = "špatně" }
	case "ordering":
		if norm == exp { r.ScoreDelta = r.Exact; r.Notes = "správně" } else if len(norm) > 0 { r.ScoreDelta = r.Partial; r.Notes = "částečně správně" } else { r.ScoreDelta = r.Wrong; r.Notes = "špatně" }
	case "scenario":
		if strings.Contains(norm, exp) { r.ScoreDelta = r.Exact; r.Notes = "správně" } else if len(norm) > 0 { r.ScoreDelta = r.Partial; r.Notes = "částečně správně" } else { r.ScoreDelta = r.Wrong; r.Notes = "špatně" }
	default:
		if norm == exp { r.ScoreDelta = r.Exact; r.Notes = "správně" } else { r.ScoreDelta = r.Wrong; r.Notes = "špatně" }
	}
	return r
