...
type state struct { repo repository.ProgressRepo; ag agent.Agent; topicIdx int; task domain.Task; correct, wrong, score int; weak map[string]int; area map[string]domain.AreaStat; topic map[string]domain.TopicStat; attempts []domain.Attempt; testRemaining int; testMode bool }

func Start(db *sql.DB) { a := app.New(); w := a.NewWindow("linux-tutor"); w.Resize(fyne.NewSize(1200, 800)); st := newState(db); w.SetContent(buildUI(st)); w.ShowAndRun() }
func newState(db *sql.DB) *state { repo := repository.ProgressRepo{DB: db}; c,w,_ := repo.Load(); st := &state{repo:repo, ag:agent.New("internal/catalog/lpic.json"), weak:map[string]int{}, area:map[string]domain.AreaStat{}, topic:map[string]domain.TopicStat{}, attempts:[]domain.Attempt{}, correct:c, wrong:w, testRemaining:60}; if len(st.ag.Catalog.Topics)>0 { st.task = st.ag.Generate(st.ag.Catalog.Topics[0].Code) }; return st }
...
%s", code, lessonText(code, ttitle))); practice.SetText(fmt.Sprintf("[%s] %s", s.task.Kind, s.task.Prompt)); stats.SetText(renderStats(s)); feedback.SetText(""); answer.SetText("") }; if len(s.ag.Catalog.Topics)>0 { refresh(s.ag.Catalog.Topics[0].Code) }
	...
	testStart := widget.NewButton("Start 60s test", func() { s.testMode = true; s.testRemaining = 60; refresh(s.ag.Catalog.Topics[0].Code); timer.SetText("60s") })
	lessonTab := container.NewVBox(lesson)
	practiceTab := container.NewVBox(practice, answer, widget.NewButton("Submit", submit), feedback)
	testTab := container.NewVBox(timer, testStart, widget.NewButton("Submit test answer", submit), widget.NewLabel("Timer is managed in the TUI version and as a simple countdown state in the GUI."))
	...
	return container.NewBorder(container.NewVBox(title), nil, topicList, nil, tabs)
}
