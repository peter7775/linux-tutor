...
func (r ProgressRepo) SaveAttempt(topic, prompt, answer, notes string, score int) error { _, err := r.DB.Exec(`INSERT INTO attempts(topic_code, prompt, answer, notes, score_delta, created_at) VALUES (?, ?, ?, ?, ?, datetime('now'))`, topic, prompt, answer, notes, score); return err }
